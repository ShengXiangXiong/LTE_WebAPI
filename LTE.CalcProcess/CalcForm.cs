            ...
            for (int b = 0; b < bids.Count; b++)
            {
                ...
                if (!disAngle.Keys.Contains(id)) // 被遮挡，不构成初级 beam
                {
                    for (int i = vertex.Count - 1, j = 0; i >= 0; j = i, i--)
                    {
                        ...
                        Polygon polygon = new Polygon(v, 4, k++, id, false);
                        room.addPolygon(ref polygon);

                        topPlane.Add(new Vector3((float)vertex[i].X, (float)vertex[i].Y, height));
                        cntV += 5;
                    }
                    Polygon topFace = new Polygon(ref topPlane, k++, id, false);
                ...
                else
                {
                    ...
                    for (int i = vertex.Count - 1, j = 0; i >= 0; j = i, i--)
                    {
                        ...
                        if (i >= tmp.minIndex && j >= tmp.minIndex && i <= tmp.maxIndex && j <= tmp.maxIndex)  // 相对小区可见
                            polygon = new Polygon(v, 4, k++, id, true);
                        else
                            polygon = new Polygon(v, 4, k++, id, false);
                        room.addPolygon(ref polygon);
                    ...
                    Polygon topFace;
                    if (height <= source.Z)
                        topFace = new Polygon(ref topPlane, k++, id, true);   // 相对小区可见
                    else
                        topFace = new Polygon(ref topPlane, k++, id, false);
                    room.addPolygon(ref topFace);
                }
