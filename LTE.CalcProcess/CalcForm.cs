...
namespace LTE.CalcProcess
{
    public partial class CalcForm : Form
    {
        ...
        private void beamPath(ref Dictionary<int, TriangleBound> disAngle)
        {
            ...
            #region 构造 room 面的顶点应该是逆时针，使得面的法向量向外
            Room room = new Room();
            int k = 0;
            uint cntV = 0;
            ...
            foreach (KeyValuePair<int, List<Geometric.Point>> item in TINInfo.TINVertex)
            {
                ...
                Vector3[] v = { new Vector3((float)item.Value[2].X, (float)item.Value[2].Y, (float)item.Value[2].Z), // 顶点必须是逆时针顺序
        ...
        private void afterCalc()
        {
            ...
            else
            {
                ...
                if (isRecordReray && this.interAnalysis.reRays.Count > 0)
                {
