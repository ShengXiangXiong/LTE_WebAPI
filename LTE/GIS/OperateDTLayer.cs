    ...
    public class OperateDTLayer
    {
        ...
        public void constuctDTGrids()
        {
            ...
            foreach (DataRow dataRow in gridTable.Rows)
            {
                ...
                if (!(float.TryParse(dataRow[2].ToString(), out x1) && float.TryParse(dataRow[3].ToString(), out y1) && float.TryParse(dataRow[4].ToString(), out x2) && float.TryParse(dataRow[5].ToString(), out y2) && float.TryParse(dataRow[6].ToString(), out recePower)))
                    continue;
