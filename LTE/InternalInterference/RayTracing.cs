...
namespace LTE.InternalInterference
{
    public class RayTracing
    {
        ...
        */
        ...
        /// <summary>
        /// 2019.5.10 求射线与均匀栅格侧面的入交点和出交点，如果两个交点高度均比当前均匀栅格内所有的建筑物高，则不需要进行碰撞检测
        /// </summary>
        /// <param name="origin">射线起点</param>
        /// <param name="dir">射线方向</param>
        /// <param name="grid">均匀栅格</param>
        /// <param name="buildingids">均匀栅格内的建筑物 ID</param>
        /// <returns>true：射线不需要与当前均匀栅格内的建筑物进行碰撞检测；false：需要进行碰撞检测</returns>
        public bool rayCrossAGrid3D(Point origin, Vector3D dir, Grid3D grid, ref List<int> buildingids)
        {
        ...
        public NodeInfo getInfoOfLineCrossAccGrid(Point origin, Vector3D dir, Grid3D grid, ref List<Point> buildingPolygon, ref RayType rayType)
        {
            ...
            if (buildingids == null) return null;

            // 2019.5.10 加速
            if (this.rayCrossAGrid3D(origin, dir, grid, ref buildingids))
                return null;
            ...
            foreach (var keyvalue in keyvalues)
            {
                ...
                if (crossWithTop == null)
                {
                    ...
                    {
                        this.getSidePlanePoint(origin, dir, crossEdge[0], crossEdge[1], normal, buildingHeight, out crossWithSidePlane, ref rayType);
                        ...
                        {
                            ray = new NodeInfo(origin, crossWithSidePlane, crossEdge[0], crossEdge[1], keyvalue.Key, buildingHeight, normal, rayType, Vector3D.getAngle(ref dir, ref normal) - Math.PI / 2.0);
                ...
                else
                {
                    if (GridHelper.getInstance().checkPointXYZInGrid(crossWithTop))
                    {
                        ...
                        if (rayType == RayType.HDiffraction)
                        {
                            ...
                            ray = new NodeInfo(origin, crossWithTop, polygonPoints[topEdgeIndex], polygonPoints[nextEdgePointIndex], keyvalue.Key, buildingHeight, edge, rayType, Vector3D.getAngle(ref dir, ref edge));
                            buildingPolygon = polygonPoints;
                        }
                        else
                        {
                            ...
                            ray = new NodeInfo(origin, crossWithTop, tmp1, tmp2, keyvalue.Key, buildingHeight, normal, rayType, Vector3D.getAngle(ref dir, ref normal) - Math.PI / 2.0);
                            buildingPolygon = polygonPoints;
                        }
                    }
                }
                //t4 = DateTime.Now;
                //Console.WriteLine("get cross point ray: {0} ms\n", (t4 - t3).TotalMilliseconds.ToString());
                if (ray != null)
                    break;
            }
            //t2 = DateTime.Now;
            //Console.WriteLine("get Ray function: {0} ms\n", (t2 - t1).TotalMilliseconds.ToString());
        ...
        public void rayTracingFirstOri(Point originPoint, Point endPoint, List<NodeInfo> rayList, SourceInfo sourceInfo, RayType rayType, int type, double coverageRadius)
        {
            ...
            //MessageBox.Show(string.Format("RayAzimuth {0}", sourceInfo.RayAzimuth));
            ...
            //MessageBox.Show(string.Format("dir {0},{1},{2}", dir.XComponent, dir.YComponent, dir.ZComponent));
            ...
            do
            {
                ...
                //MessageBox.Show(string.Format("curAccGrid {0},{1},{2}", curAccGrid.gxid, curAccGrid.gyid, curAccGrid.gzid));
            ...
            switch (type)
            {
                ...
                case 2:
                    ...
                    // 直接到达建筑物顶面
                    if (Math.Abs(ray.CrossPoint.Z - endPoint.Z) < 0.5)
                    {
                        this.rayCountDirB++;
                        Vector3D normal = new Vector3D(0, 0, 1);
                ...
                case 4:
                    ...
                    // 2017.8.2 直接到达建筑物棱边
                    ok1 = Math.Abs(ray.CrossPoint.X - endPoint.X) < 0.5;
                    ok2 = Math.Abs(ray.CrossPoint.Y - endPoint.Y) < 0.5;
                    ok3 = Math.Abs(ray.CrossPoint.Z - endPoint.Z) < 0.5;
                    if (ok1 && ok2 && ok3)
                    {
                        rayList.Add(ray);  // 先把初级直射线加进来
        ...
        public void rayTracing(Point originPoint, Vector3D dir, List<NodeInfo> rayList, SourceInfo sourceInfo, RayType rayType, double coverageRadius)
        {
            ...
            //计算
            //if ((diffractionCounter >= this.DiffrectionLevel) || (reflectionCounter >= this.ReflectionLevel))
            //{
            //    this.CalcOutDoorRayStrength(rayList, sourceInfo.RayAzimuth, sourceInfo.RayInclination, false);
            //    return;
            //}
        ...
        /// <param name="rayType">射线类型</param>
        public void getSidePlanePoint(Point start, Vector3D dir, Point point1, Point point2, Vector3D normal, double height, out Point crossPoint, ref RayType rayType)
        {
            //求射线与面的交点
            ...
            if (crossPoint.X > x2 || crossPoint.X < x1 || crossPoint.Y > y2 || crossPoint.Y < y1 || crossPoint.Z > height || crossPoint.Z <= 0)
            {
                crossPoint = null;
            }
